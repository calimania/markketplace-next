...
async function proxyGet(req: NextRequest, upstreamPath: string) {
  ...
  const upstreamUrl = new URL(upstreamPath, markketplace.api);
...
// GET /api/crm/orders?storeRef=<slug>&status=<status>&q=<search>&page=1&pageSize=25
export async function GET(req: NextRequest) {
  try {
    return await proxyGet(req, '/api/crm/orders');
  } catch (error) {
    console.error('[crm/orders] GET failed:', error);
