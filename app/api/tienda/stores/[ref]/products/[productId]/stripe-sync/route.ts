...
async function handler(
  req: NextRequest,
  context: { params: Promise<{ ref: string; productId: string }> },
) {
  ...
  try {
    ...
    const upstreamUrl = buildUpstreamUrl(req, `/api/products/${productId}/stripe_sync`);
    return await proxyToUpstream(req, upstreamUrl, auth.token);
  } catch (error) {
    console.error('Tienda product Stripe sync proxy error:', error);
