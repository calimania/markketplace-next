...
async function handler(req: NextRequest, context: { params: Promise<{ ref: string; contentType: string }> }) {
  ...
  try {
    const { ref, contentType } = await context.params;
    ...
    const upstreamUrl = buildUpstreamUrl(req, `/api/tienda/stores/${ref}/content/${contentType}`);
