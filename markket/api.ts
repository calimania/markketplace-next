...
export class StrapiClient {
  ...
  private buildUrl(options: EnhancedFetchOptions): string {
    ...
    if (paginate?.limit)
      params.append("pagination[limit]", paginate.limit.toString());
    if (paginate?.page)
      params.append("pagination[page]", paginate.page.toString());
    if (paginate?.pageSize)
      params.append("pagination[pageSize]", paginate.pageSize.toString());
    if (sort) params.append("sort", sort);

    const url = new URL(
      `api/${contentType}?${filterString}&${params.toString()}`,
      this.baseUrl
    );
